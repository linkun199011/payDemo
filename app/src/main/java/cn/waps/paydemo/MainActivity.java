...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		// 初始化统计器(必须调用)
		PayConnect.getInstance("379959297c825c9723e56e1617eaff1c", "WAPS", this);
		
		//微信支付初始化(不接入微信支付请忽略)
		PayConnect.setWxAppID("wxa496b0774f4ee5af");
		...
		goodsDesc = "购买" + goodsName + "VIP，去广告并且开放所有权限。";
		...
		submitButton.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				try {
					...
					PayConnect.getInstance(MainActivity.this).pay(MainActivity.this, orderId, userId, price,
	...
	private class MyPayResultListener implements PayResultListener {
		...
		public void onPayFinish(Context payViewContext, String orderId,
			...
			if (resultCode == 0) {
				...
				PayConnect.getInstance(MainActivity.this).confirm(orderId,payType);
			} else {
